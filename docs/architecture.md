...
The staging directory always has a `.tmp-` prefix while in progress. The USB directory never gets the final name until the rename succeeds.
...
This catches: filesystem corruption on the local SSD, files modified or truncated during staging (race condition with a live system), rsync from agent VMs that produced a truncated bundle. If verification fails, the backup aborts before anything reaches USB.

### Atomic commit to USB

Data is written to `<FINAL_DIR>.tmp/` and renamed to `<FINAL_DIR>/` only after the full rsync completes. This means:

- The USB never contains a directory that looks complete but isn't
...
The trap is detached during the rename window so the cleanup handler doesn't delete the staging directory while the rename is in flight. `sync` is called before and after the rename to flush the kernel's write-behind cache.

rsync uses `--whole-file` (no delta algorithm) because this is a local copy, not a network transfer. `--inplace` is deliberately not used — it writes in-place and leaves a partial file indistinguishable from a complete one after power loss.

### Post-transfer manifest verification

After the USB rename, `verify_manifest_on_usb` re-reads and verifies all checksums from `MANIFEST.sha256` on the USB. This catches USB write errors, bad sectors, write-behind cache failures, and silent bit rot on the USB medium. Both verifications must pass before PABS proceeds to offsite sync or rotation.

### UUID targeting

`check_usb_mounted` reads the UUID of whatever is mounted at `USB_MOUNT` and compares it to `TARGET_UUID`. The backup aborts if they don't match. This prevents writing to the wrong drive if a second USB stick is connected and automounted at the same path, or if the mount point was reconfigured.

---

## Space checks

Space estimation runs before staging begins. `_estimate_backup_kb` sums the disk usage of all paths that will be backed up, then adds 512 MB per configured VM agent as a heuristic for agent bundle size (deliberately over-estimating to account for HAOS snapshots and large Docker volume exports). Both `check_local_stage_space` and `check_usb_space` apply a 20% margin on top.

If USB space is insufficient, PABS attempts auto-recovery: it purges the oldest backup and re-checks. It will not purge if only one backup remains. If still not enough space after purging, the backup aborts.
...
| `system-state/apt-holds.txt` | `apt-mark showhold` | Reference for held packages |
| `system-state/apt-manual.txt` | `apt-mark showmanual` | Reference for manually-installed packages |
| `system-state/proxmox-version.txt` | `pveversion --verbose` | Target version for reinstall |
| `system-state/kernel-version.txt` | `uname -r` | Reference |
...
ZFS pool creation itself requires manual `zpool create` — these files are reference snapshots, not a pool restore mechanism.
...
├── source types/<type>.sh           ← provides do_backup()
│
├── do_backup()
...
When `VM_AGENT_MAX_PARALLEL > 1`, agents are launched in the background with `_run_agent &`. A `_wait_one` polling loop enforces the parallelism cap — it blocks until a slot is free before launching the next agent.

Each parallel worker writes to a private temp file. Results are assembled in order after all workers complete. The shared log file (`$LOG`) is written via `tee -a`, which is atomic for short writes on Linux.

---

## Secret redaction

`config.sh` is written into each backup as a restore reference. Secrets are redacted before writing — the original `config.sh` on the host is never modified.

**Explicit keys** (always redacted):

- `DISCORD_WEBHOOK`, `NOTIFY_EMAIL`, `PORTAINER_TOKEN`
- `RCLONE_ENCRYPTION_PASSWORD`, `RCLONE_ENCRYPTION_SALT`

**Generic catch-all** (values ≥ 4 chars, variable name matches):

- Contains `Password`, `Secret`, `_TOKEN`, `_KEY`, or `WEBHOOK` (case-insensitive for Password/Secret)
...
This creates or overwrites the named rclone remote `pabs_crypt_runtime` at runtime. Because rclone's crypt remote is deterministic — the same password + salt always produces the same key material (derived via scrypt) — overwriting the config each run is idempotent and existing offsite data remains readable.

`rclone obscure` converts the plaintext password to rclone's config storage format (simple XOR, not the actual encryption key). The actual AES-256 key is derived from the passwords using scrypt. All subsequent rclone operations use `pabs_crypt_runtime:` — including retention pruning functions which operate through the crypt layer so they see decrypted directory names.
...
Lists all `YYYY-MM-DD_*` directories under `BACKUP_ROOT`, sorts oldest-first, and deletes until count ≤ `KEEP_BACKUPS`. Guards against `KEEP_BACKUPS=0` by never deleting the directory created in the current run. Rotation happens after offsite sync, so USB always has the freshest backup when the offsite sync fires.
...
2. **Storage pass** — iterates oldest-first marking more for deletion until `rclone size` reports usage below `RCLONE_MAX_STORAGE_GB`

Then applies `RCLONE_KEEP_MIN` as a safety gate: trims the delete list from the newest end until survivors ≥ `RCLONE_KEEP_MIN`. Deletions use `rclone purge` (recursive directory delete). Failures on individual deletions are non-fatal.
...
│   Non-fatal: logs a warning if curl fails
│
...
Discord fires on both success and failure. Email fires on failure only, to avoid inbox noise on every weekly success.

---

## Staging directory size estimates

Typical full backup with 4 VM agents:
...
Total typical range: **300 MB – 3 GB**. The space estimator in `lib/preflight.sh` uses 512 MB per configured agent as a conservative heuristic, then adds a 20% margin.
